...
public partial class WebUI_Stock_MoveView : BasePage
{
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
        ...
        if (Count > 0)
        {
            JScript.Instance.ShowMessage(this.updatePanel, "该移库单号已被其它单据使用，请调整后再删除！");
            return;
        }
    ...
    protected void btnCheck_Click(object sender, EventArgs e)
    {
        ...
        if (this.btnCheck.Text == "审核")
        {
            ...
            for (int i = 0; i < dtSub.Rows.Count; i++)
            {
                ...
                if (count > 0)
                {
                    JScript.Instance.ShowMessage(this.updatePanel, "货位 " + dtSub.Rows[i]["CellCode"].ToString() + "已经被其它单据锁定，不能移库！");
                    return;
                }
                count = bll.GetRowCount("Cmd_Cell", string.Format("CellCode='{0}' and (IsLock=1 or IsActive=0)", dtSub.Rows[i]["NewCellCode"]));
                if (count > 0)
                {
                    JScript.Instance.ShowMessage(this.updatePanel, "货位 " + dtSub.Rows[i]["NewCellCode"].ToString() + "已经被其它单据锁定，不能移库！");
                    return;
                }
            }
